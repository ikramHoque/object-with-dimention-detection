# sam3__moge2__sam2__sonnet5 — system design
...
**The question this pipeline answers:** Everything we expect to be best, together. Run this LAST, after the single-factor runs point at it.

| slot | model | licence |
|---|---|---|
| detector | `sam3` — SAM 3 (concept segmentation) | Meta SAM licence |
| depth | `moge2` — MoGe-2 ViT-L (metric) | MIT |
| segmenter | `sam2` — SAM 2.1 Hiera-Large | Apache-2.0 |
...
flowchart TD
    ...
    S1 -->|"frames, long edge 1024px"| S2["<b>Stage 2 · Detect</b><br/>sam3<br/><i>finds and names objects</i>"]
    S1 -->|"the WHOLE frame, never crops"| S3["<b>Stage 3 · Depth</b><br/>moge2<br/><i>metres per pixel</i>"]

    S2 -->|"boxes"| SEG["<b>Stage 2b · Segment</b><br/>sam2<br/><i>box to pixel mask</i>"]
    SEG -->|"masks"| S4
    SEG -->|"boxes + labels"| S4
...
| 2 | **Detect** `sam3` | frame + 26 prompts | boxes + matched text | An open-vocabulary detector returns *matched text*, not a class id, so the label is normalised to one prompt. Where it cannot pick one it returns **no** label rather than guessing — a wrong label forces the wrong size class whatever the geometry says. |
| 2b | **Segment** `sam2` | boxes | one mask per object | A box around a sofa also contains floor and wall, and those pixels get measured as part of the sofa. Masks measure the object only. Measured effect: door error 16.5% -> 6.8%. |
...
sequenceDiagram
    ...
    participant D as sam3
    participant SEG as sam2
...
| raw m3 far above class-snapped | boxes contain wall and floor | masks are on; check they are not too tight |
