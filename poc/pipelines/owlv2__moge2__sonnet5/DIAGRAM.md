# owlv2__moge2__sonnet5 — system design
...
**The question this pipeline answers:** Does OWLv2's long-tail training beat Grounding DINO on unusual furniture?

| slot | model | licence |
|---|---|---|
| detector | `owlv2` — OWLv2 (base, ensemble) | Apache-2.0 |
...
flowchart TD
    ...
    S1 -->|"frames, long edge 1024px"| S2["<b>Stage 2 · Detect</b><br/>owlv2<br/><i>finds and names objects</i>"]
...
| 2 | **Detect** `owlv2` | frame + 26 prompts | boxes + matched text | An open-vocabulary detector returns *matched text*, not a class id, so the label is normalised to one prompt. Where it cannot pick one it returns **no** label rather than guessing — a wrong label forces the wrong size class whatever the geometry says. |
