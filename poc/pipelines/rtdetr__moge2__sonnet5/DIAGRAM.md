# rtdetr__moge2__sonnet5 — system design
...
**The question this pipeline answers:** How much do we lose by giving up open vocabulary for speed and a clean licence?

| slot | model | licence |
|---|---|---|
| detector | `rtdetr` — RT-DETR R50 (Objects365 + COCO) | Apache-2.0 |
...
flowchart TD
    ...
    S1 -->|"frames, long edge 1024px"| S2["<b>Stage 2 · Detect</b><br/>rtdetr<br/><i>finds and names objects</i>"]
...
| 2 | **Detect** `rtdetr` | frame + 26 prompts | boxes + matched text | An open-vocabulary detector returns *matched text*, not a class id, so the label is normalised to one prompt. Where it cannot pick one it returns **no** label rather than guessing — a wrong label forces the wrong size class whatever the geometry says. |
