# NOSHIP_AGPL__yoloe__moge2__sonnet5 — system design
...
**The question this pipeline answers:** Does YOLOE deliver real-time open vocabulary AND masks? AGPL - comparison only.

| slot | model | licence |
|---|---|---|
| detector | `yoloe` — YOLOE-v8L-seg (via Ultralytics) | AGPL-3.0 or paid Ultralytics Enterprise |
...
flowchart TD
    ...
    S1 -->|"frames, long edge 1024px"| S2["<b>Stage 2 · Detect</b><br/>yoloe<br/><i>finds and names objects</i>"]
...
| 2 | **Detect** `yoloe` | frame + 26 prompts | boxes + matched text | An open-vocabulary detector returns *matched text*, not a class id, so the label is normalised to one prompt. Where it cannot pick one it returns **no** label rather than guessing — a wrong label forces the wrong size class whatever the geometry says. |
