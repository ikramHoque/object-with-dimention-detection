# rtdetr__da3_metric__nocls — system design
...
**The question this pipeline answers:** rtdetr + da3_metric
...
| classifier | _none — Method B only, no API key needed_ | - |
...
flowchart TD
    ...
    S6["<b>Stage 6 · skipped</b><br/>no classifier<br/><i>Method B alone, no API key</i>"]
    S6 -.-> S7
...
| 6 | **Recognise** _skipped_ | the frame | size classes + counts | No classifier in this pipeline, so there is no Method A and no cross-check — and no API key needed. This is the run that answers "how good is pure geometry alone?" |
